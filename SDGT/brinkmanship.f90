...
do while(.not.success)
	...
	inquire(file=fileout(1:ilastout)//".str",exist=filelives)
	if(filelives)then
		print*,' file ',filein(1:ilastout)//".str",' alreadys exists; overwrite(y/n)? '
		read(5,'(a)')ychar
		if(ychar=='n' .or. ychar=='N')cycle
		open(unit=unitout,file=fileout(1:ilastout)//".str",status='old')
	else
			open(unit=unitout,file=fileout(1:ilastout)//".str",status='new')
	end if
