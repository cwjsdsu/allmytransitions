...
do while(.not.success)
	...
	inquire(file=filein(1:ilastin)//"usdbBminus.str",exist=filelives)
	if(.not.filelives)then
		print*,' file ',filein(1:ilastin)//"usdbBminus.str",' does not exist '
		cycle
	end if
	open(unit=unitin,file=filein(1:ilastin)//"usdbBminus.str",status='old')
	success = .true.
...
!	end if
	...
	inquire(file=filein(1:ilastin)//"usdbBminusBrink.str",exist=filelives)
	if(filelives)then
		print*,' file ',filein(1:ilastin)//"usdbBminusBrink.str",' alreadys exists; overwrite(y/n)? '
		read(5,'(a)')ychar
		if(ychar=='n' .or. ychar=='N')cycle
		open(unit=unitout,file=filein(1:ilastin)//"usdbBminusBrink.str",status='old')
	else
			open(unit=unitout,file=filein(1:ilastin)//"usdbBminusBrink.str",status='new')
	end if
